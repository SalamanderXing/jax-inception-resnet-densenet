...
class TrainerModule:
    ...
    def init_optimizer(self, num_epochs, num_steps_per_epoch):
        ...
        if (
            opt_class == optax.sgd and "weight_decay" in self.optimizer_hparams
        ):  # wd is integrated in adamw
            transf.append(
                optax.add_decayed_weights(self.optimizer_hparams.pop("weight_decay"))
            )
        optimizer = optax.chain(
            *transf, opt_class(lr_schedule, **self.optimizer_hparams)
        )
    ...
    def test_classifier(
        ...
        *,
        ...
        assert trainer.checkpoint_exists()  # Skip training if pretrained model exists
        trainer.load_model(pretrained=True)
        return trainer.eval_model(test_loader)
